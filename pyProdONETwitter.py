...
filename = 'ONE_TWITTER_request_times.txt'
...
for n in range(1, 51):
	...
	f.write("ONE_TWITTER FirstRequest " + str(n))
	...
	passman.add_password(None, url1, 'ONE_TWITTER\olsena', 'Intern@1')
	...
	#f1 = open('ONE_TWITTER_OUT1.html', 'w')
	#f1.write(html)
	#f1.close()
	...
	f.write("ONE_TWITTER SecondRequest " + str(n))
	nowStart = datetime.datetime.now()
	strStartTime = nowStart.strftime("%H:%M:%S-%f")
	f.write(" ___ Start Time ||||| " + strStartTime + " ||||| ")
	...
	passman = urllib2.HTTPPasswordMgrWithDefaultRealm()
	passman.add_password(None, url2, 'ONE_TWITTER\olsena', 'Intern@1')
	urllib2.install_opener(urllib2.build_opener(urllib2.HTTPBasicAuthHandler(passman)))
	...
	#write it out to file - debug purposes
	#f1 = open('ONE_TWITTER_OUT2.html', 'w')
	#f1.write(html)
	#f1.close()
